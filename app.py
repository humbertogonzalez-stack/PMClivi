...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Clivi 2.0: Comando de Crecimiento", layout="wide", initial_sidebar_state="collapsed")
...
if st.session_state.view == "papelera":
    ...
    if df_trash.empty: st.info("La papelera está vacía.")
    else:
...
tab_kanban, tab_cuadernos = st.tabs(["🚀 Tablero de Operaciones", "📒 Cuadernos de Conocimiento"])
    ...
    if st.session_state.editing_task_id and not df.empty:
        ...
        with st.form("edit"):
            ...
            if eb1.form_submit_button("💾 Guardar"):
                ...
                if enueva_img:
                    final_img_url = upload_image(enueva_img)
...
# ==========================================
# PESTAÑA 2: CUADERNOS DE CONOCIMIENTO
# ==========================================
with tab_cuadernos:
    st.header("Gestión de Conocimiento")
    st.markdown("Documenta estrategias, aprendizajes, guiones y procesos.")
            ...
            if st.form_submit_button("Guardar en Inteligencia", type="primary"):
                if c_titulo and c_cont:
                    supabase.table("clivi_cuadernos").insert({
                        "titulo": c_titulo, "autores": c_autores, "contenido": c_cont
                    }).execute()
                    st.success("Cuaderno guardado exitosamente.")
                    st.rerun()

    with col_c2:
        st.subheader("Cuadernos Recientes")
        try:
            res_c = supabase.table("clivi_cuadernos").select("*").order("fecha_creacion", desc=True).execute()
            ...
            st.warning("Ejecuta el comando SQL para crear la tabla de Cuadernos en Supabase para habilitar esta sección.")
